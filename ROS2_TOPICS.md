...
DS18B20 수온 변환을 시작한 뒤 변환 대기 시간 동안 pH, DO, 탁도 ADC를 40회씩
교차 샘플링한다. 수온 결과를 마지막에 읽고 pH와 DO를 보정 계산하므로 수온
보정과 기존 샘플 수를 유지하면서 전체 측정을 약 1초 안에 완료하도록 구성했다.
...
| `measurement_ms` | 한 번의 수질 측정과 계산에 걸린 시간(ms) |
| `temp_c` | 수온(°C) |
| `ph` | 계산된 pH |
